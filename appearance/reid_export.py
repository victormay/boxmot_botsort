...
from boxmot_botsort.appearance import export_formats
from boxmot_botsort.appearance.reid_model_factory import get_model_name, load_pretrained_weights, build_model, get_nr_classes
from boxmot_botsort.utils import WEIGHTS, logger as LOGGER

from boxmot_botsort.appearance.exporters.base_exporter import BaseExporter
from boxmot_botsort.appearance.exporters.torchscript_exporter import TorchScriptExporter
from boxmot_botsort.appearance.exporters.onnx_exporter import ONNXExporter
from boxmot_botsort.appearance.exporters.openvino_exporter import OpenVINOExporter
from boxmot_botsort.appearance.exporters.tflite_exporter import TFLiteExporter
from boxmot_botsort.appearance.exporters.tensorrt_exporter import EngineExporter
...
def main():
    ...
    if args.half:
        assert args.device != "cpu", "--half only compatible with GPU export, i.e. use --device 0"

    model = build_model(
        get_model_name(args.weights),
        num_classes=get_nr_classes(args.weights),
        pretrained=not (args.weights and args.weights.is_file() and (args.weights.suffix == ".pt" or args.weights.suffix == ".pth")),
        use_gpu=args.device,
    ).to(args.device)

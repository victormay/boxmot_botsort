...
from boxmot_botsort.appearance.reid_auto_backend import ReidAutoBackend
from boxmot_botsort.motion.cmc import get_cmc_method
from boxmot_botsort.motion.kalman_filters.xywh_kf import KalmanFilterXYWH
from boxmot_botsort.trackers.botsort.basetrack import BaseTrack, TrackState
from boxmot_botsort.utils.matching import (embedding_distance, fuse_score,
                                   iou_distance, linear_assignment)
from boxmot_botsort.utils.ops import xywh2xyxy, xyxy2xywh
from boxmot_botsort.trackers.basetracker import BaseTracker
# from boxmot_botsort.utils import PerClassDecorator


class STrack(BaseTrack):
    shared_kalman = KalmanFilterXYWH()

    def __init__(self, det, feat=None, feat_history=50, max_obs=50, ts=0):
        # wait activate
        ...
        self.cls_hist = defaultdict(lambda :0)
        ...
        self.start_ts = ts
        self.end_ts = ts
        self.tss = deque([], maxlen=feat_history)
    ...
    def update_cls(self, cls, conf):
        self.cls_hist[cls] += conf
        max_cls = ""
        max_conf = 0
        for i, j in self.cls_hist.items():
            if j > max_conf:
                max_cls = i
                max_conf = j
        self.cls = max_cls
    ...
    def re_activate(self, new_track, frame_id, new_id=False, ts=0):
        ...
        self.tss.append(self.end_ts - self.start_ts)
        self.start_ts = ts
        self.end_ts = ts
...
class BoTSORT(BaseTracker):
    def __init__(
        ...
        self.with_reid = True if model_weights else False
        if self.with_reid:
    ...
    def update(self, dets: np.ndarray, img: np.ndarray, embs: np.ndarray = None, ts: float = 0) -> np.ndarray:
        ...
        if len(dets) > 0:
            ...
            if self.with_reid:
                ...
                detections = [STrack(det, f, max_obs=self.max_obs, ts=ts) for (det, f) in zip(dets_first, features_high)]

            else:
                detections = [STrack(det, max_obs=self.max_obs, ts=ts) for (det) in np.array(dets_first)]
        else:
            detections = []
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(detections[idet], self.frame_count, ts)
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_count, new_id=False, ts=ts)
                refind_stracks.append(track)

        """ Step 3: Second association, with low conf detection boxes"""
        if len(dets_second) > 0:
            """Detections"""
            detections_second = [STrack(dets_second, max_obs=self.max_obs, ts=ts) for dets_second in dets_second]
        else:
            detections_second = []
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(det, self.frame_count, ts)
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_count, new_id=False, ts=ts)
                refind_stracks.append(track)
        ...
        for itracked, idet in matches:
            unconfirmed[itracked].update(detections[idet], self.frame_count, ts)
